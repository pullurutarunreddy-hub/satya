...
import { PreviewChitPlanDialog } from '@/components/admin/preview-chit-plan-dialog';
...
export type ChitPlanFormValues = z.infer<typeof chitPlanSchema>;

export default function CreateChitPlanPage() {
  const router = useRouter();
  const { toast } = useToast();
  const [isPreviewOpen, setIsPreviewOpen] = useState(false);
  const [planData, setPlanData] = useState<ChitPlanFormValues | null>(null);
  ...
  const handlePreview = (values: ChitPlanFormValues) => {
    setPlanData(values);
    setIsPreviewOpen(true);
  }

  const handleFinalSubmit = async (values: ChitPlanFormValues) => {
    try {
        const docData = {
            ...values,
        ...
        await addDoc(collection(db, "chit-plans"), docData as Omit<ChitPlan, 'id'>);
        toast({
            title: "Plan Created",
    ...
    <>
      <div className="container mx-auto py-12 px-4">
        ...
        <Card className="max-w-4xl mx-auto">
          ...
          <CardContent>
            <Form {...form}>
              <form className="space-y-6" onSubmit={form.handleSubmit(handlePreview)}>
