...
const bankSchema = z.object({
  accountNumber: z.string().min(9, 'Invalid account number').max(18, 'Invalid account number').optional().or(z.literal('')),
...
export function ApplicationForm({ planId }: { planId: string }) {
  ...
  const onSubmit = async (data: FieldValues) => {
    ...
    try {
        ...
        // Note: In a real app, you would upload files to Firebase Storage and save the URLs, not the file objects.
    ...
    <div className="space-y-8">
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          ...
          {currentStep === 2 && (
             <div className="space-y-6">
                <h2 className="text-xl font-semibold">Bank Details (Optional)</h2>
                <FormField name="accountNumber" control={form.control} render={({ field }) => (
                    <FormItem><FormLabel>Bank Account Number</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormMessage /></FormItem>
                )} />
